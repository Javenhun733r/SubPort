      ...
      <form @submit.prevent="submitForm" class="form-content">
        ...
        <div class="form-group">
          ...
          <div class="input-prefix-wrapper">
            ...
            <input v-model="newAuthor.username" type="text" id="username" placeholder="унікальний_нікнейм" class="input-field with-prefix" required @input="clearMessage"/>
          </div>
        </div>

        <div class="form-group">
          <label for="name" class="form-label">Ім’я автора <span class="required-star">*</span></label>
          <input v-model="newAuthor.name" type="text" id="name" placeholder="Як вас представляти" class="input-field" required @input="clearMessage"/>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="file-input-custom-wrapper">
            ...
            <label for="avatarFile" class="file-input-button">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 inline-block" viewBox="0 0 20 20" fill="currentColor"><path d="M5.5 13a3.5 3.5 0 01-.369-6.98 4 4 0 117.753-1.977A4.5 4.5 0 1113.5 13H11V9.414l-1.293 1.293a1 1 0 01-1.414-1.414l3-3a1 1 0 011.414 0l3 3a1 1 0 01-1.414 1.414L13 9.414V13H5.5z" /><path d="M9 13h2v5a1 1 0 11-2 0v-5z" /></svg>
              <span>{{ avatarFile ? 'Змінити файл' : 'Обрати файл' }}</span>
            </label>
            <span v-if="avatarFile" class="file-name-display">{{ avatarFile.name }}</span>
          </div>
          <p v-if="avatarFile && avatarFile.size > 2 * 1024 * 1024" class="text-xs text-red-400 mt-1">Файл завеликий. Макс. розмір 2MB.</p>
        </div>
        ...
        <fieldset class="form-group socials-fieldset">
          ...
          <div class="socials-container">
            <div v-for="(social, index) in newAuthor.socials" :key="index" class="social-input-group">
              ...
              <input v-model="social.link" type="url" placeholder="https://..." class="input-field social-link-input" aria-label="Посилання"/>
              <button type="button" @click="removeSocial(index)" class="button-remove-social" title="Видалити соцмережу" aria-label="Видалити соцмережу">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
              </button>
            </div>
          </div>
          <button @click="addSocial" type="button" class="button-add-social">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1.5 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
            Додати ще
          </button>
        </fieldset>
        ...
        <div class="form-group">
          ...
          <textarea v-model="newAuthor.bio" id="bio" rows="5" placeholder="Чим ви займаєтесь, ваші інтереси, чим можете бути корисні..." class="input-field"></textarea>
        </div>
...
export default {
  name: 'AddAuthorPage', // Додамо ім'я компоненту
  data() {
    return {
      newAuthor: {
        username: '',
        ...
        bio: '',
        genre: '',
        socials: [] // Ініціалізуємо порожнім масивом
      },
      avatarFile: null,
      // selectedSocials: [], // Видалено, будемо використовувати newAuthor.socials напряму
      socialNetworks: ['Instagram', 'Facebook', 'Telegram', 'Twitter / X', 'YouTube', 'TikTok', 'LinkedIn', 'Patreon', 'Behance', 'Dribbble', 'ArtStation', 'GitHub', 'Website'],
    ...
    async submitForm() {
      ...
      try {
        ...
        if (err.response && err.response.data && err.response.data.message) {
          ...
          this.formMessage = { type: 'error', text: `Не вдалося надіслати заявку (код: ${err.response.status}). Перевірте введені дані.` };
        } else if (err.request) {
...
.add-author-page-wrapper {
  min-height: 100vh;
  display: flex;
  flex-direction: column; /* Дозволяє футеру бути внизу, якщо контенту мало */
  align-items: center;
  justify-content: center; /* Центрує по вертикалі, якщо форма не займає всю висоту */
  padding: 2rem 1rem;
  font-family: "Raleway", sans-serif;
  color: #e0e0e0;
  /* Якщо глобальний фон не встановлено, розкоментуйте наступний рядок */
...
.form-container {
  ...
  padding: 2rem 2.5rem; /* Tailwind p-8 sm:p-10 */
  border: 1px solid rgba(0, 247, 255, 0.15);
  box-shadow: 0 8px 32px rgba(0, 0, 0, 0.35);
  max-width: 700px; /* Збільшено ширину для вмісту */
  width: 100%;
  margin: 2rem 0; /* Додає відступи зверху/знизу, якщо сторінка довша за екран */
}
...
.form-content {
  ...
  gap: 1.25rem; /* Tailwind space-y-5 */
}
...
.legend-label {
  padding: 0 0.5rem;
  margin-left: 0.5rem;
  font-size: 0.9rem; /* Трохи більша легенда */
  color: #00f7ff;
  font-weight: 500;
}

.required-star {
  color: #ff7b7b; /* Світло-червоний */
}

.input-field {
  width: 100%;
  padding: 0.875rem 1rem; /* Tailwind py-3 px-4 */
  border-radius: 8px; /* Tailwind rounded-lg */
  background-color: rgba(10, 15, 35, 0.65); /* Темніший, менш прозорий */
...
.input-field::placeholder { color: #6b7f99; } /* Яскравіший плейсхолдер */
.input-field:focus {
  outline: none;
  border-color: #00f7ff;
  background-color: rgba(10, 15, 35, 0.85);
  box-shadow: 0 0 0 3px rgba(0, 247, 255, 0.2); /* Яскравіший фокус */
}

.input-prefix-wrapper { position: relative; display: flex; align-items: center; }
.input-prefix-text { position: absolute; left: 1rem; color: #718096; pointer-events: none; }
.input-field.with-prefix { padding-left: 2.25rem; }

.file-input-custom-wrapper { display: flex; align-items: center; gap: 1rem; }
...
.file-input-button {
  ...
  background-color: rgba(0, 247, 255, 0.1); color: #00f7ff;
  border: 1px solid rgba(0, 247, 255, 0.3); border-radius: 8px;
  cursor: pointer; font-weight: 500; transition: all 0.3s ease;
}
.file-input-button:hover { background-color: rgba(0, 247, 255, 0.2); border-color: rgba(0, 247, 255, 0.5); }
.file-name-display { font-size: 0.875rem; color: #a0aec0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex-shrink: 1; }
...
.socials-container { display: flex; flex-direction: column; gap: 1rem; }
.social-input-group { display: flex; gap: 0.75rem; align-items: center; }
.social-select-wrapper { flex: 1 1 40%; }
.social-select { padding-right: 2.25rem; }
.social-link-input { flex: 1 1 60%; }

.select-wrapper { position: relative; width: 100%; }
.select-arrow-container { position: absolute; right: 0.875rem; top: 50%; transform: translateY(-50%); pointer-events: none; color: rgba(0, 247, 255, 0.7); }
.select-arrow-icon { width: 1.125rem; height: 1.125rem; }
.small-arrow { width: 0.9rem; height: 0.9rem; }

.button-remove-social {
  padding: 0.6rem; color: #ff7b7b; background-color: transparent;
  border: none; border-radius: 6px; cursor: pointer;
  transition: color 0.3s ease, background-color 0.3s ease;
}
.button-remove-social:hover { color: #ff5252; background-color: rgba(255, 0, 0, 0.1); }

.button-add-social {
  margin-top: 1rem; padding: 0.625rem 1rem; border-radius: 8px;
  background-color: transparent; color: #00f7ff;
  border: 1px dashed rgba(0, 247, 255, 0.5); /* Яскравіша рамка */
  font-weight: 500; cursor: pointer; transition: all 0.3s ease;
  align-self: flex-start;
}
.button-add-social:hover { background-color: rgba(0, 247, 255, 0.1); border-style: solid; border-color: rgba(0, 247, 255, 0.7); }

.submit-button-container { text-align: center; margin-top: 1.5rem; }
.cta-button {
...
.cta-button:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0, 220, 220, 0.25); background: linear-gradient(90deg, #23ffff, #538dff); }
.cta-button:disabled { opacity: 0.6; cursor: not-allowed; transform: none; box-shadow: none; }

.button-loader { width: 20px; height: 20px; border: 2px solid #121828; border-bottom-color: transparent; border-radius: 50%; display: inline-block; box-sizing: border-box; animation: button-rotation 1s linear infinite; }
@keyframes button-rotation { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }

.form-message { padding: 0.85rem 1rem; border-radius: 8px; margin-bottom: 1.5rem; text-align: center; font-size: 0.9rem; font-weight: 500; }
.message-success { color: #c1f0d9; background-color: rgba(40, 167, 69, 0.15); border: 1px solid rgba(40, 167, 69, 0.25); }
.message-error { color: #ffc5c5; background-color: rgba(255, 0, 0, 0.1); border: 1px solid rgba(255, 0, 0, 0.2); }
</style>