    ...
    <div class="form-container">
      ...
      <form @submit.prevent="handlePasswordResetRequest" class="form-content">
        ...
        <div class="form-group" v-if="!formMessage.type || formMessage.type === 'error'">
          ...
          <input type="email" id="email" v-model="email" class="input-field" placeholder="example@email.com" required @input="clearMessage">
        </div>
...
export default {
  ...
  data() {
    return {
      email: "",
      isLoading: false,
      formMessage: { type: '', text: '' }, // type: 'success' or 'error'
    };
  },
  methods: {
    clearMessage() {
    ...
    async handlePasswordResetRequest() {
      ...
      try {
        // Замініть URL на ваш реальний ендпоінт для скидання пароля
        const response = await axios.post("http://localhost:8081/forgot-password", { email: this.email });

        this.formMessage = { type: 'success', text: response.data.message || 'Інструкції для відновлення пароля надіслано на вашу пошту!' };
        this.email = ""; // Очистити поле після успішного запиту
...
.form-main-title {
  font-size: 1.75rem; /* Зменшено для цієї форми */
  font-weight: 600;
  color: #00f7ff;
...
.cta-button {
  ...
  align-items: center;
  justify-content: center;
  margin-top: 0.5rem; /* Відступ зверху, якщо є інпути */
}
...
.form-message {
  ...
  border-radius: 8px;
  /* margin-bottom: 1.25rem; -- Прибрано, оскільки повідомлення тепер єдине в формі */
  text-align: center;
  font-size: 0.9rem;
  font-weight: 500;
  line-height: 1.5;
